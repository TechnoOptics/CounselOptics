...
const LANE_META: Record<Lane, { name: string; blurb: string }> = {
  attention: { name: 'Needs attention', blurb: 'New and flagged - triage these first' },
...
const PRIORITY_TONE: Record<string, string> = {
  ...
  Normal: 'bg-ink-200 dark:bg-forest-700 text-ink-700 dark:text-cream-100/80',
...
export default async function CounselIntakePage() {
  ...
  type Row = (typeof intakes)[number] & {
    _lane: Lane;
    _priority: string;
    _grade: string | null;
    _folder: string;
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <section className="space-y-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          {replyCount > 0 && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-gold-500/15 ring-1 ring-gold-500/40 px-2.5 py-1 text-[12px] font-semibold text-gold-700 dark:text-gold-200">
        ...
        {intakes.length === 0 ? (
          <p className="card p-5 text-[13px] text-ink-500 dark:text-cream-100/55 italic">
            No requests yet. New requests filed from the employee hub

...
-- afterwards, are the owner's steps.
--
-- Read that banner as an unverified claim, not as a fact CI stands behind.
-- Seven headers in this directory have already been found asserting the
-- wrong applied state in both directions, and nothing can contradict them:
-- the schema-drift gate self-skips while the SUPABASE_DB_URL repo secret is
-- unset, so it has never executed a comparison and a stale fingerprint
-- passes green. See scripts/schema/README.md, "Current status". Until that
-- secret exists these banners are the only record there is, so whoever runs
-- the migration has to come back and correct this one.
-- =========================================================================
--
-- WHAT THIS IS FOR
--

...
export async function generateCasePdf(input: {
  ...
  defenseAdvice?: DefenseAdvice | null;
}): Promise<Buffer> {
...
function writePdf(
  ...
  // Defense plan (if present)
  if (defenseAdvice) {
    doc.addPage();
    drawDefenseAdvice(doc, defenseAdvice);
  }
  ...
  if (plans.length > 0) {
    doc.addPage();
    drawExhibitPlan(doc, plans, exhibits);
  }
...
function drawCoverPage(
  ...
  doc.text('COUNSELOPTICS · CASE PACKET', x, y, { characterSpacing: 2 });
...
function drawDefenseAdvice(doc: Doc, advice: DefenseAdvice) {
  resetToContentTop(doc);
  section(doc, 'Defense planning (pro se)');
  ...
  list(doc, 'Procedural posture & deadlines', advice.proceduralPosture);
  list(doc, 'Evidence to gather to support your defense', advice.evidenceToGather);
  list(doc, 'Risk factors', advice.riskFactors);
  list(doc, 'When you should hire a lawyer', advice.whenToHireLawyer);
  list(doc, 'Topics to research', advice.resourceTopics);
  list(doc, 'Questions for an attorney consultation', advice.questionsForAttorney);
  ...
  doc
    ...
    .fontSize(9)
    .fillColor(COLOR.muted)
    .text(advice.disclaimer, MARGIN, doc.y, { width: CONTENT_WIDTH });
}

// ---------------------------- Exhibit plan --------------------------------

function drawExhibitPlan(doc: Doc, plans: ExhibitPlanItem[], exhibits: Exhibit[]) {
  resetToContentTop(doc);
  section(doc, 'Exhibit plan');
  doc
    .font('Helvetica')
    .fontSize(10.5)
    .fillColor(COLOR.muted)
    .text('Suggested exhibit list for this case. Each slot can be filled with an uploaded file.', MARGIN, doc.y, {
      width: CONTENT_WIDTH,
    });
  gap(doc, 14);
  ...
  for (const p of plans) {
    ...
    const filled = p.filledByExhibitId
      ...
      : null;
    const statusText = filled ? `FILLED by ${filled.fileName}` : 'NOT UPLOADED';
    ...
    if (p.description) {
      ...
      doc.text(p.description, MARGIN, doc.y + 2, { width: CONTENT_WIDTH });
    }
    gap(doc, 12);
  }
}
...
function drawDisclaimer(doc: Doc) {
  ...
  doc
    ...
    .fillColor(COLOR.ink)
    .text(
      'CounselOptics provides legal information, case organization tools, document summaries, and AI-assisted issue spotting. CounselOptics does not provide legal advice, does not represent users, and does not create an attorney-client relationship.',
      MARGIN,
      doc.y,
      { width: CONTENT_WIDTH, align: 'left' },
    );
  gap(doc, 12);
  doc.text(
    'Any classifications, summaries, or possible offense categories generated by the platform are informational only and may be incomplete or incorrect depending on the facts, jurisdiction, evidence, and applicable law. Users should consult a licensed attorney in their jurisdiction before making legal decisions, contacting law enforcement, filing a case, responding to a claim, or taking any action based on information generated by CounselOptics.',
    MARGIN,
    doc.y,
    { width: CONTENT_WIDTH, align: 'left' },
  );
...
function drawFooter(doc: Doc, pageIndex: number, total: number, caseTitle: string) {
  ...
  doc.text('CounselOptics', MARGIN, y - 4, { width: 200, align: 'left' });

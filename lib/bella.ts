...
- "Draft an NDA" / "write a demand letter" / "make me an engagement letter": follow the existing list_document_templates -> confirm inputs -> draft full text in your reply -> draft_document flow. When the firm has uploaded a letterhead under /counsel/settings, the rendered PDF paints that letterhead across the top of page 1 automatically; you don't need to do anything special. If no letterhead is uploaded, the PDF falls back to a text-only header with the firm name. If the user wants their letterhead on the PDF and it's not there, point them at /counsel/settings.

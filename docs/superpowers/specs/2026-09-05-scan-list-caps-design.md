# A cap on the parties, dates and amounts a scan lists
...
Then, on review: cap parties too, in the same PR.
...
1. **One number, `SCAN_LIST_CAP = 25`, exported from `lib/ai.ts`, applied to
   parties, dates and amounts alike.** A fill at the cap for all three lists
   costs about a thousand tokens (a date entry about twenty, an amount about
   seven, a party name about eight), leaving the budget to the summary and
   the other fields. The review prompt pastes every stored party, date and
...
2. **Stated in the three places the model sees the lists.** The Parties,
   Dates and Amounts rules in `SCAN_SYSTEM`, `maxItems` on all three arrays
   in `SCAN_TOOL`,
...
3. **What to keep when the document holds more.** Parties: the ones the
   document is between or about (the parties to the matter, the issuer, the
   signers, the court). Dates: the ones that identify the document (issue,
   due, hearing and signing dates, first and last entries). Amounts: totals,
   balances, the largest entries, first and last. A spreadsheet read: first
   and last rows, totals or balances, the largest entries, and the payees or
   counterparties that appear most often.
...
6. **Parties are capped too.** The first cut left them out as outside the
   ask; the owner asked for them in review. A payee column on a tracker has
   exactly the shape the dates and amounts had, and one rule for three lists
   is simpler than one rule for two and an exception.
...
number between 10 and 40, all three arrays carry it as `maxItems`, all
three rules state it, the summary instruction is present, the spreadsheet rule names
first and last rows and the row count, and a fill at the cap fits inside the
call's `max_tokens`. Five mutations (schema cap dropped on amounts, then on
parties; spreadsheet rule dropped; parties rule cap dropped; cap raised to
200) each turned the file red before being restored.

...
| Config changes | Apply immediately | The legal team can edit the popup text and questions at any time, so always render from `GET /config` and never cache longer than a day |
...
- **Verification discipline:** keep rejecting unsigned, stale (>5 min), or
  ...
  drop) on repeated signature failures, since that pattern means a secret
...
- We version the partner API by path (`/api/partner/v1/…`). Breaking
  ...
  notice; `v1` additions (new fields, new event types) are non-breaking, so
...
3. **Full rollout (week 2+):** enable for everyone on `zinpro.com` /
   `zinpro.app`. No provisioning step; the first ticket creates the account.

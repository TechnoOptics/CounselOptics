...
**Expect** `200` with (this is the exact live shape, and it matches
ADVOTTIC-INTEGRATION-CONTRACT.md §5.1; an earlier draft of this runbook
showed a `config`-wrapped shape that does NOT exist, so code to this):
...
| Employee gets no emails | Their address only, so check spam; deliveries are from `invites@advottic.com` (displayed as the firm) | We can check send logs |

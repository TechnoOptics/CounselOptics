...
function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="space-y-1">
      <p className="text-xs font-medium text-ink-600 dark:text-cream-100/70">{label}</p>
    ...
    <div className="space-y-5">
      <div className="space-y-3">
        ...
        <p className="text-xs text-ink-600 dark:text-cream-100/70 leading-relaxed">
          Paste this as the <em>Tenant URL</em> in your identity provider
          (Microsoft Entra ID, Okta, and any SCIM 2.0 directory). It provisions
          and deprovisions people in your firm directory automatically as you add
          or remove them in your IdP.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-ink-200 p-4 dark:border-forest-700/40">
        ...
        <p className="mt-1 text-xs text-ink-600 dark:text-cream-100/70 leading-relaxed">
          Generate a token, then paste it into your IdP&rsquo;s <em>Secret
          Token</em> field. For your security we show it only once and store only
          a hash, so copy it before you leave this page. Generating a new token
          lets you rotate; the previous token keeps working until you remove it.
        </p>
        ...
        >
          {busy ? 'Generating...' : token ? 'Generate another token' : 'Generate token'}
        </button>
      </div>
    </div>
  );
